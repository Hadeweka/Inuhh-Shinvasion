...
# Important classes for the game
require "lib/Main/Residuum.rb"
require "lib/Main/Projectile.rb"
require "lib/Main/Inuhh_Main.rb"
require "lib/Main/World_Inuhh.rb"
require "lib/Main/Map.rb"
require "lib/Main/StdTextField.rb"

class Game < Window
  attr_reader :map, :inuhh, :sounds, :level, :valimgs, :font, :shi_cry_heard, :minigame_flag, :minigame_lifes
  attr_accessor :messages, :residues, :halted, :enemies, :triggers, :debug_strength_flag, :gems, :timer
  ...
  def play_sound(name, volume = 1, speed = 1, looping = false)
    if name.index("shi cry") then
      ...
      t = Time.now
      if t.hour == 0 && t.min == 0 && rand(1000) == 0 then
  ...
  def update
    ...
    if @title_screen then
      ...
      if !@halted && !@semi_halted then
        @timer += 1 if @timer && @timer < 60*60*60 - 1
      ...
      if @nm then
        ...
        if datas.size > 0 then
          translate(0.0, -@achievement_scroller) do
            ...
            milestone_array[Milestones::Minigame_Nerd] = perfect_minigames >= 5
            ...
            c += 30
            milestone = Milestones::Minigame_Nerd
            if perfect_minigames > 0 then
              @font.draw(Strings::A_Perfect_Minigames, 80, c, ZOrder::UI, 1.0, 1.0)
              @font.draw(perfect_minigames.to_s, 300, c, ZOrder::UI, 1.0, 1.0, (milestone_array[milestone] ? 0xffdddd00 : 0xff00ffff))
            else
              @font.draw(Strings::Unknown, 80, c, ZOrder::UI, 1.0, 1.0, 0xff888888)
            end
            if milestone_array[milestone] then
        ...
        if @timer then
          minutes_raw = (@timer / 60 / 60).to_i
          seconds_raw = (@timer / 60).to_i % 60
          frames_raw = @timer % 60
          minutes_formatted = minutes_raw < 10 ? "0#{minutes_raw}" : minutes_raw.to_s
          seconds_formatted = seconds_raw < 10 ? "0#{seconds_raw}" : seconds_raw.to_s
          second_percents_raw = (frames_raw / 60 * 100).to_i
          second_percents_formatted = second_percents_raw < 10 ? "0#{second_percents_raw}" : second_percents_raw.to_s
          @font.draw(minutes_formatted + ":" + seconds_formatted + ":" + second_percents_formatted, 500, 0, ZOrder::UI, 2.0, 2.0, 0xffffffff)
        end
        ...
        if @display_wait && @display_text then
          if @hedgehound then
            ...
            compass_pieces = (other_option(Other::Compass) ? other_option(Other::Compass).size : 0)
            if compass_pieces >= 4 && WORLD_7_ALLOWED then
              @font.draw(WORLD_7_DIALOG, 60, 270, ZOrder::UI_Extended, 1.5, 1.5, 0xffffffff)
              @font.draw(Strings::Hedge_Compass, 400, 270, ZOrder::UI_Extended, 1.5, 1.5, 0xff00ff00)
            elsif compass_pieces >= 1 && WORLD_7_ALLOWED then
              @font.draw(WORLD_7_U_DIALOG, 60, 270, ZOrder::UI_Extended, 1.5, 1.5, 0xffffffff)
              @font.draw(Strings::Hedge_Unknown_Compass, 400, 270, ZOrder::UI_Extended, 1.5, 1.5, 0xffff0000)
            end
  ...
  def button_down(id) # Some self-references here... maybe?
    ...
    if @title_screen then
      ...
      if Keys::Gems3.include?(id) && @i_collectibles[Objects::Gem] >= GEM_LIFES_5 then
        @i_collectibles[Objects::Gem] -= GEM_LIFES_5
        @i_stats[Stats::Lifes] += 5
      end
      if Keys::Gems4.include?(id) && @i_collectibles[Objects::Gem] >= GEM_LIFES_10 then
      ...
      if @hedgehound then
        ...
        if Keys::Gems1.include?(id) && sc >= hedge_energy then
          ...
          @display_wait = false
          save_state
        elsif Keys::Gems6.include?(id) && compass_pieces >= 4 && WORLD_7_ALLOWED then
          @unlocked_levels.push("6-1") if !@unlocked_levels.index("7-1")
          @level = "7-1-1"
          change_world_img(@level.split("-")[0])
          @text_flag = true if !@display_wait
          @display_wait = false
